...
// 声明一个全局数组变量
var globaImglArray = Array.from({length: 42}, (_, i) => i + 1);
var globaImglArrayFlag = false;
...
*/
if (!globaImglArrayFlag){
  initImglArray();
  globaImglArrayFlag = true;
}
var randomNum = Math.floor(Math.random() * 99) + 1;
globaImglArrayLen.push(randomNum);
var iiic = globaImglArray[globaImglArrayLen.length-1];
thisPic.src="/rimg/"+iiic+".jpg";
}


function initImglArray(){
  for (var i =0,len = globaImglArray.length; i<len; i++){
    var rand = parseInt(Math.random()*len);
    var temp = globaImglArray[rand];
    globaImglArray[rand] == globaImglArray[i];
    globaImglArray[i] = temp;
  }
}

function timeline(){

$(".XMT_tline_main .XMT_tline_year>h2>a").click(function(e){
	e.preventDefault();
	$(this).parents(".XMT_tline_year").toggleClass("close");
});
}